...
class DetectMultiBackend(nn.Module):
    ...
    def __init__(self, weights="yolov5s.pt", device=torch.device("cpu"), dnn=False, data=None, fp16=False, fuse=True):  # noqa: B008
...
class ASPP(nn.Module):
    """Atrous Spatial Pyramid Pooling (ASPP) module with parallel dilated convolutions."""
...
class CoTBlock(nn.Module):
    """Contextual Transformer Block integrating 3x3 static context convolution with dynamic self-attention."""
...
class CoTBottleneck(nn.Module):
    """Bottleneck module using CoTBlock instead of standard 3x3 convolution."""

    def __init__(self, c1, c2, shortcut=True, g=1, e=0.5):
        super().__init__()
        c_ = int(c2 * e)
        g = 1 if (not isinstance(g, int) or isinstance(g, bool) or g < 1) else g
        self.cv1 = Conv(c1, c_, 1, 1)
...
class CoT3(C3):
    """C3 module with CoTBottleneck for contextual self-attention feature extraction."""

    def __init__(self, c1, c2, n=1, shortcut=True, g=1, e=0.5):
        if isinstance(shortcut, (int, float)) and not isinstance(shortcut, bool):
            e = shortcut
            shortcut = g if isinstance(g, bool) else True
            g = 1
        super().__init__(c1, c2, n, shortcut, g, e)
        c_ = int(c2 * e)
        g = 1 if (not isinstance(g, int) or isinstance(g, bool) or g < 1) else g
